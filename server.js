...
async function initDB() {
    ...
    try {
        ...
        // Create admin_users table if not exists (DON'T DROP)
        await client.query(`
        ...
        // Create default superadmin ONLY if no admins exist
        ...
        if (existingAdmin.rows[0].cnt === 0) {
            ...
            const result = await client.query(
                'INSERT INTO admin_users (username, email, password_hash, role) VALUES ($1, $2, $3, $4) RETURNING id, username, role',
                ['admin', 'admin@ghostwire.io', hashedPassword, 'superadmin']
            );
            console.log('  ✅ Default admin created:', result.rows[0].username);
        } else {
            console.log('  ✅ Admin users already exist (' + existingAdmin.rows[0].cnt + ' found)');
        }
...
app.put('/api/auth/profile', authenticateToken, async (req, res) => {
    try {
        ...
        await pool.query('UPDATE admin_users SET email = $1 WHERE id = $2', [email || '', req.user.id]);
...
app.put('/api/admins/:id', authenticateToken, async (req, res) => {
    try {
        ...
        const result = await pool.query(
            'UPDATE admin_users SET email = $1, role = $2, is_active = $3 WHERE id = $4 RETURNING id, username, email, role, is_active',
...
app.get('/api/whitelist', authenticateToken, async (req, res) => {
    try {
        const result = await pool.query(`
            ...
            FROM whitelist w 
            LEFT JOIN admin_users a ON w.added_by = a.id 
...
app.delete('/api/whitelist/:id', authenticateToken, async (req, res) => {
    try {
        await pool.query('UPDATE whitelist SET is_active = false WHERE id = $1', [req.params.id]);
...
app.get('/api/blacklist', authenticateToken, async (req, res) => {
    try {
        const result = await pool.query(`
            ...
            FROM blacklist b 
            LEFT JOIN admin_users a ON b.added_by = a.id 
...
app.delete('/api/blacklist/:id', authenticateToken, async (req, res) => {
    try {
        await pool.query('UPDATE blacklist SET is_active = false WHERE id = $1', [req.params.id]);
...
// ============================================
// GENERIC RECORD CRUD (FIXED WITH PK MAPPING)
// ============================================
...
app.get('/api/records/:table/:id', authenticateToken, async (req, res) => {
    try {
        ...
        console.log(`📖 GET record: table=${table}, pk=${pk}, id=${id}`);
...
app.put('/api/records/:table/:id', authenticateToken, async (req, res) => {
    try {
        ...
        console.log(`✏️ UPDATE record: table=${table}, pk=${pk}, id=${id}`);
...
app.delete('/api/records/:table/:id', authenticateToken, async (req, res) => {
    try {
        ...
        console.log(`🗑️ DELETE record: table=${table}, pk=${pk}, id=${id}`);
        
        const result = await pool.query(`DELETE FROM ${table} WHERE ${pk} = $1 RETURNING ${pk}`, [id]);
...
app.get('/api/dashboard/stats', authenticateToken, async (req, res) => {
    try {
        ...
        return res.json({
            total_users: 0, total_wallets: 0, open_positions: 0,
            active_channels: 0, today_trades: 0, whitelist_count: 0,
            blacklist_count: 0, admin_count: 1
        });
    }
});

// Get trade count for last 7 days
app.get('/api/dashboard/trade-volume', authenticateToken, async (req, res) => {
    try {
        ...
        for (let i = 6; i >= 0; i--) {
            ...
            days.push(dayName);
            counts.push(found ? found.trade_count : 0);
        }
...
app.get('/api/dashboard/user-registrations', authenticateToken, async (req, res) => {
    try {
        ...
        for (let i = 6; i >= 0; i--) {
            ...
            days.push(dayName);
            counts.push(found ? found.user_count : 0);
        }
...
app.get('/api/dashboard/user-distribution', authenticateToken, async (req, res) => {
    try {
        const result = await pool.query(`
            SELECT 
                CASE WHEN token_percent >= 2 THEN 'Whales'
                     WHEN token_percent >= 0.5 THEN 'Holders'
                     ELSE 'Traders' END as category,
                COUNT(*)::int as count
            FROM token_verifications WHERE is_verified = true
            GROUP BY category ORDER BY count DESC
        `);
        return res.json({ labels: result.rows.map(r=>r.category), data: result.rows.map(r=>r.count) });
    } catch (error) {
        return res.json({ labels: ['Whales','Holders','Traders'], data: [0,0,0] });
    }
});
...
app.get('/api/dashboard/recent-trades', authenticateToken, async (req, res) => {
    try {
        const result = await pool.query(`
            SELECT th.created_at, u.username, th.trade_type,
                   LEFT(th.token_address,12)||'...' as token, ROUND(th.amount::numeric,4) as amount
...
async function startServer() {
    console.log('🚀 GHOSTWIRE Admin Server v2.0\n');
    ...
    app.listen(PORT, () => {
        ...
        console.log(`  👤 admin / GhostWire@2026`);
        console.log(`══════════════════════════════\n`);
    });
}
