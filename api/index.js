...
if (url === '/api/dashboard/stats' && method === 'GET') {
    const result = await getPool().query(`
        SELECT 
            ...
            (SELECT COALESCE(SUM(total_value), 0)::numeric(20,3) FROM trade_history WHERE DATE(created_at)=CURRENT_DATE AND trade_type = 'sell') as today_volume_sol,
            (SELECT COALESCE(SUM(total_value), 0)::numeric(20,3) FROM trade_history WHERE trade_type = 'sell') as total_volume_sol
    `);
    return sendJSON(res, 200, result.rows[0] || {});
}
